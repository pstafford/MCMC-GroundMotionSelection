function [Mean_Sa Sd_Sa ] = CampbellBozorgnia_2008_SA (M, T, Rrup, Rjb, Ztor, delta, lambda, Vs30, Zvs, arb)
% This code is originally created by Yoshifumi Yamamoto, 11/10/08 with deleting T==1000 case.
% Stanford University
...
if (length(find(period==T(i)))==0)
     T_low= max(period(find(period<T(i))));
     T_hi= min(period(find(period>T(i))));
%calculate spectral acceleration
      [Mean_SaLow,Sd_SaLow]=CampbellBozorgnia_2008_SA(M,T_low,Rrup,Rjb, Ztor, delta, lambda, Vs30, Zvs, arb);
      [Mean_SaHi,Sd_SaHi]=CampbellBozorgnia_2008_SA(M,T_hi,Rrup,Rjb, Ztor, delta, lambda, Vs30, Zvs, arb);
%interpolate 
      x=[log(T_low),log(T_hi)];
      y_Mean=[log(Mean_SaLow),log(Mean_SaHi)];
      y_Sd=[Sd_SaLow,Sd_SaHi];
      Mean_Sa(i)=exp(interp1(x,y_Mean, log(T(i))));
      Sd_Sa(i)=interp1(x,y_Sd, log(T(i)));
 else
      ip=find(period==T(i));
      [Mean_Sa(i),Sd_Sa(i)]=CampbellBozorgnia_2008_SA_sub(M, ip, Rrup, Rjb, Ztor, delta, lambda, Vs30, Zvs, arb, ffltz, fhngr, Frv, Fnm, fhngm);
 end
end

function [Sa sigma] = CampbellBozorgnia_2008_SA_sub (M, ip, Rrup, Rjb, Ztor, delta, lambda, Vs30, Zvs, arb, ffltz, fhngr, Frv, Fnm, fhngm)
...
if Vs30 < k1(ip);
    A1100 = CampbellBozorgnia_2008_SA (M, 0, Rrup,  Rjb,Ztor, delta, lambda, 1100, Zvs, arb);
